...
# ── 페이지 진입 시 필요한 데이터 병렬 사전 로드 ─────
from concurrent.futures import ThreadPoolExecutor

with st.spinner("데이터 불러오는 중..."):
    with ThreadPoolExecutor(max_workers=4) as _exe:
        _fut_fx       = _exe.submit(get_ticker_data, "USDKRW=X")
        _fut_holdings = _exe.submit(get_current_holdings)
        _fut_closed   = _exe.submit(get_closed_positions)
        _fut_cash     = _exe.submit(get_cash_summary)

        fx_result     = _fut_fx.result()
        _holdings_pre = _fut_holdings.result()
        _closed_pre   = _fut_closed.result()
        _cash_pre     = _fut_cash.result()

FX = fx_result["price"] if fx_result.get("ok") else 1330.0
...
# ─────────────────────────────────────────────────
with tab1:
    holdings = _holdings_pre  # 병렬 사전 로드 결과 재사용
...
# ─────────────────────────────────────────────────
with tab2:
    closed = _closed_pre  # 병렬 사전 로드 결과 재사용
